...
- **🚫🚫🚫 严格禁止同层提取（最重要）**：
  - **绝对不能在同一层内提取概念关系**
  - **禁止L2-L2、L3-L3、L4-L4等任何同层连接**
  - **同层的概念之间不能有任何连接关系**
  - **如果发现两个概念在同一层，它们之间不能有连接**
...
5. **拒绝无效连接**：
   ...
   - ❌ 拒绝L1-L3、L1-L4（跨层）
   - ❌ 拒绝L2-L4（跨层）
   - **🚫🚫🚫 绝对拒绝L2-L2、L3-L3、L4-L4、L5-L5等任何同层连接**
   - ❌ 拒绝L3-L1、L4-L1、L4-L2（反向跨层）
   - **⚠️ 如果两个概念在同一层，它们之间绝对不能有连接**
6. **层级完整性检查**：
   - ✓ 确保L1层至少有1个节点
   - ✓ 确保L2层至少有1个节点
   - ✓ 确保L3层至少有1个节点
...
2. **概念词不限制字数**：根据内容需要，可以是简短的概念名称，也可以是更详细的描述性文字，以准确表达概念内容为准（但必须来源于文本）
...
**🚫🚫🚫 同层连接禁止检查（必须逐一核对）：**
✓ **绝对禁止任何L2-L2连接**（同一层的L2概念之间不能有连接）
✓ **绝对禁止任何L3-L3连接**（同一层的L3概念之间不能有连接）
✓ **绝对禁止任何L4-L4连接**（同一层的L4概念之间不能有连接）
✓ **绝对禁止任何L5-L5连接**（同一层的L5概念之间不能有连接）
✓ **如果两个概念在同一层，它们之间绝对不能有任何连接**
...
请开始输出三元组（记住：**严格按文本提取，禁止自行生成**，只能从高层到低层提取，绝对禁止反向、同层和跨层提取，L1→L2、L2→L3、L3→L4单向流动，**确保每个相邻层之间都有足够的三元组连接（L1→L2至少4个，L2→L3至少6个，L3→L4至少6个，但前提是文本中确实存在这些内容）**，内容适合约300字介绍）：`;
    }
