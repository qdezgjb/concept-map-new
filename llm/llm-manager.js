...
- **⚠️ L2、L3、L4层必须严格等于4、5或6个节点，不能是3个或7个等其他数字！**
- **⚠️ 超过节点数量限制或不符合随机不重复要求将导致概念图布局混乱，必须严格控制！**
- **内容长度要求：约300字左右**
- **层级完整性要求：每一层都必须有节点**
- **⭐ 相邻层连接强制要求（最重要）：**
...
2. **L2（第二层）**：主要分类或维度（必须是4、5、6个节点中的一个，随机选择）
3. **L3（第三层）**：具体分类或子维度（必须是4、5、6个节点中的一个，随机选择，不能与L2层重复）
4. **L4（第四层）**：具体细节或实例（必须是4、5、6个节点中的一个，随机选择，不能与L2、L3层重复）

## 提取规则：
1. 将文本按句号、分号、逗号等标点符号分割成行
2. 分析每行内容，确定概念的层级归属
3. **🔴 严格控制每层节点数量（必须严格遵守随机不重复规则）**：
   - **L1层：有且只有1个节点（绝对不能有第2个！）**
   - **L2层：必须是4、5、6个节点中的一个（随机选择，严格等于4、5或6，不能是其他数字）**
   - **L3层：必须是4、5、6个节点中的一个（随机选择，严格等于4、5或6，不能是其他数字，且不能与L2层重复）**
   - **L4层：必须是4、5、6个节点中的一个（随机选择，严格等于4、5或6，不能是其他数字，且不能与L2、L3层重复）**
   - **🔴🔴🔴 关键要求：L2、L3、L4三层的节点数量必须从{4, 5, 6}中随机选择，且三层之间不能重复。**
4. **层级完整性要求**：
...
3. **🔴 检查节点数量限制（严格遵守，这是硬性约束）**：
   ...
   - **L2层节点数量必须严格等于4、5或6个，不能是3个、7个或其他数字**
   - **L3层节点数量必须严格等于4、5或6个，不能是3个、7个或其他数字，且不能与L2层重复**
   - **L4层节点数量必须严格等于4、5或6个，不能是3个、7个或其他数字，且不能与L2、L3层重复**
   - **🔴🔴🔴 关键检查：L2、L3、L4三层的节点数量必须从{4, 5, 6}中随机选择，且三层之间不能重复**
   - **⚠️ L1层超过1个节点会导致布局严重错误，必须严格拒绝！**
   - **⚠️ L2/L3/L4层不是4、5、6个节点或三层之间有重复会导致布局混乱，必须拒绝！**
4. **验证层级关系（只允许从高到低）**：
...
## 概念要求：
1. 概念词必须简短（2-6个字）
2. 必须明确标注层级（L1、L2、L3、L4）
3. **🔴🔴🔴 关键要求：同一个概念在整个三元组列表中必须始终使用相同的层级标记！**
...
**🔴🔴🔴 L2、L3、L4三层的节点数量必须从{4, 5, 6}中随机选择，且三层之间不能重复！这是硬性约束！**
- 🔴 L1层只能有1个节点，第2个必须拒绝
- **L2层必须严格等于4、5或6个节点，不能是3个、7个或其他数字**
- **L3层必须严格等于4、5或6个节点，不能是3个、7个或其他数字，且不能与L2层重复**
- **L4层必须严格等于4、5或6个节点，不能是3个、7个或其他数字，且不能与L2、L3层重复**

请开始输出三元组（记住：🔴**L1层只能有1个节点！**，只能从高层到低层提取，绝对禁止反向、同层和跨层提取，L1→L2、L2→L3、L3→L4单向流动，**🔴🔴🔴 L2、L3、L4三层的节点数量必须从{4, 5, 6}中随机选择，且三层之间不能重复！**，**确保每个相邻层之间都有足够的三元组连接（L1→L2至少4个，L2→L3至少6个，L3→L4至少6个）**，内容适合约300字介绍）：`;
    }
...
## ⚠️ 严格数量限制（必须遵守）：
- **总节点数：13-19个**（1 + L2节点数 + L3节点数 + L4节点数，其中L2、L3、L4各为4、5或6个）
- **第一层（L1）**：必须且只有1个节点（焦点问题本身）
- **第二层（L2）**：必须是4、5或6个节点中的一个（随机选择，严格等于4、5或6，不能是其他数字）
- **第三层（L3）**：必须是4、5或6个节点中的一个（随机选择，严格等于4、5或6，不能是其他数字，且不能与L2层重复）
- **第四层（L4）**：必须是4、5或6个节点中的一个（随机选择，严格等于4、5或6，不能是其他数字，且不能与L2、L3层重复）
- **🔴🔴🔴 关键要求：L2、L3、L4三层的节点数量必须从{4, 5, 6}中随机选择，且三层之间不能重复。例如：L2=4, L3=5, L4=6 或 L2=5, L3=6, L4=4 等。**
...
# 重要说明
- **总节点数必须是13-19个**（1 + L2节点数 + L3节点数 + L4节点数，其中L2、L3、L4各为4、5或6个）
- 第一层只有1个节点（焦点问题）
- **第二层必须是4、5或6个节点中的一个（随机选择，严格等于4、5或6，不能是其他数字）**
- **第三层必须是4、5或6个节点中的一个（随机选择，严格等于4、5或6，不能是其他数字，且不能与L2层重复）**
- **第四层必须是4、5或6个节点中的一个（随机选择，严格等于4、5或6，不能是其他数字，且不能与L2、L3层重复）**
- **🔴🔴🔴 关键要求：L2、L3、L4三层的节点数量必须从{4, 5, 6}中随机选择，且三层之间不能重复。**
...
✓ nodes数组长度为13-19个
✓ layer=1的节点只有1个
✓ layer=2的节点必须是4、5或6个中的一个（随机选择，严格等于4、5或6，不能是其他数字）
✓ layer=3的节点必须是4、5或6个中的一个（随机选择，严格等于4、5或6，不能是其他数字，且不能与L2层重复）
✓ layer=4的节点必须是4、5或6个中的一个（随机选择，严格等于4、5或6，不能是其他数字，且不能与L2、L3层重复）
✓ L2、L3、L4三层的节点数量必须从{4, 5, 6}中随机选择，且三层之间不能重复
✓ 每个节点都有layer属性
...
- **总节点数：13-19个**（1 + L2节点数 + L3节点数 + L4节点数，其中L2、L3、L4各为4、5或6个）
- 第一层：1个节点（核心概念）
- 第二层：必须是4、5或6个节点中的一个（随机选择，严格等于4、5或6，不能是其他数字）
- 第三层：必须是4、5或6个节点中的一个（随机选择，严格等于4、5或6，不能是其他数字，且不能与L2层重复）
- 第四层：必须是4、5或6个节点中的一个（随机选择，严格等于4、5或6，不能是其他数字，且不能与L2、L3层重复）
- **🔴🔴🔴 关键要求：L2、L3、L4三层的节点数量必须从{4, 5, 6}中随机选择，且三层之间不能重复。**
...
要求：
- 总共13-19个概念（nodes数组长度13-19，其中L2、L3、L4各为4、5或6个）
- 必须包含layer属性（1、2、3或4）
